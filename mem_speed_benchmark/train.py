...
parser.add_argument('--multiprocessing-distributed', action='store_true',
                    help='Use multi-processing distributed training to launch '
                         ...
                         'fastest way to use Paddle for either single node or '
...
def main():
    ...
    if args.multiprocessing_distributed:
        ...
        # Use paddle.distributed.spawn to launch distributed processes: the
        # main_worker process function
        paddle.distributed.spawn(main_worker, nprocs=ngpus_per_node, args=(ngpus_per_node, args))
    else:
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if not paddle.is_compiled_with_cuda():
        ...
        if args.gpu is not None:
            ...
            model = paddle.DataParallel(model, device_ids=[args.gpu])
        else:
            model = model.cuda()
        ...
        if args.arch.startswith('alexnet') or args.arch.startswith('vgg'):
            model.features = paddle.DataParallel(model.features)
            model.cuda()
        else:
    ...
    if args.distributed:
        train_sampler = paddle.io.DistributedBatchSampler(dataset=train_dataset, batch_size=args.batch_size, shuffle=True)
    else:
        train_sampler = None
    ...
    if args.get_macs:
        ...
        if isinstance(model, QModule):
            ...
            model = model.model
        model = model.module
        input = paddle.randn(shape=[1, 3, args.input_size, args.input_size]).cuda()
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (indices, (images, target)) in enumerate(train_loader()):
        ...
        if args.gpu is not None:
            images = paddle.to_tensor(images).cuda(args.gpu, non_blocking=True)
        if paddle.is_compiled_with_cuda():
            target = paddle.to_tensor(target).cuda(args.gpu, non_blocking=True)
        # compute output
        output = model(images)
