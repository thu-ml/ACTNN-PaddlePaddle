...
from paddle.vision.models import resnet_versions
...
from paddle.fluid.dygraph.learning_rate_scheduler import LinearWarmupCosineDecayLR
# from paddle.fluid.dygraph.base import to_variable
from paddle.fluid.dygraph.nn import CrossEntropyLoss
from paddle.fluid.dygraph import DataLoader
...
def add_parser_arguments(parser):
    ...
    model_names = resnet_versions.keys()
    model_configs = models.resnet_configs.keys()

    parser.add_argument('data', metavar='DIR',
                        help='path to dataset')
    parser.add_argument('--dataset', type=str, default='imagenet')
    parser.add_argument('--data-backend', metavar='BACKEND', default='paddle',
                        choices=DATA_BACKEND_CHOICES)
    parser.add_argument('--arch', '-a', metavar='ARCH', default='resnet50',
                        help='model architecture: resnet50')
    parser.add_argument('--model-config', '-c', metavar='CONF', default='fanin',
                        help='model configs: classic')
...
def main(args):
    ...
    args.world_size = 1
    if args.distributed:
        args.gpu = args.local_rank % paddle.distributed.ParallelEnv().device_num
        paddle.set_device(args.gpu)
        dist.init_parallel_env()
        args.world_size = paddle.distributed.ParallelEnv().world_size
    if args.amp and args.fp16:
        print("Please use only one of the --fp16/--amp flags")
        exit(1)
    if args.seed is not None:
        print("Using seed = {}".format(args.seed))
        paddle.seed(args.seed + args.local_rank)
        def _worker_init_fn(id):
            paddle.seed(args.seed + args.local_rank + id)
    else:
        def _worker_init_fn(id):
            pass
    if args.fp16:
        assert paddle.is_compiled_with_cuda(), "fp16 mode requires CUDA support."
    ...
    if args.pretrained_weights:
        if os.path.isfile(args.pretrained_weights):
            ...
            pretrained_weights = paddle.load(args.pretrained_weights)
        else:
    ...
    if args.resume2:
        if os.path.isfile(args.resume2):
            ...
            checkpoint2 = paddle.load(args.resume2)
    ...
    if args.dataset == 'cifar10':
        get_train_loader = get_paddle_train_loader_cifar10
        get_val_loader = get_paddle_val_loader_cifar10
        get_debug_loader = get_paddle_debug_loader_cifar10
        QScheme.num_samples = 50000     # NOTE: only needed for use_gradient
    elif args.data_backend == 'pytorch':
        get_train_loader = get_paddle_train_loader
        get_val_loader = get_paddle_val_loader
        get_debug_loader = get_paddle_val_loader
    ...
    train_loader, train_loader_len = get_train_loader(args.data, args.batch_size, args.num_classes, args.mixup > 0.0,
                                                      workers=args.workers, fp16=args.fp16)
    if args.mixup != 0.0:
        train_loader = MixUpWrapper(args.mixup, args.num_classes, train_loader)
    val_loader, val_loader_len = get_val_loader(args.data, args.batch_size, args.num_classes, False,
                                                workers=args.workers, fp16=args.fp16)
    debug_loader, debug_loader_len = get_debug_loader(args.data, args.batch_size, args.num_classes, False,
                                                      workers=args.workers, fp16=args.fp16)
    ...
    if args.distributed:
        dist.init_parallel_env()
        model_and_loss = paddle.DataParallel(model_and_loss)
