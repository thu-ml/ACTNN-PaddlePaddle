...
try:
    from paddle.distributed import DistributedBatchSampler, DistributedSampler, init_parallel_env
    from paddle.nn.parallel import DistributedDataParallel as DDP
    from paddle.fluid.dygraph import to_variable
    from paddle.fluid.dygraph.fp16_utils import *
    from paddle.fluid.dygraph import amp
    from paddle.distributed import DistributedDataParallel
    from apex.fp16_utils import *
    from apex import amp
except ImportError:
    raise ImportError("Please install paddlepaddle-gpu or apex from https://www.github.com/nvidia/apex to run this example.to run this example.")
...
class ModelAndLoss(nn.Layer):
    def __init__(self, arch, num_classes, loss, pretrained_weights=None, cuda=True, fp16=False):
        ...
        if pretrained_weights is not None:
            ...
            model.set_state_dict(pretrained_weights)
        if cuda:
            model = model.cuda()
        if fp16:
...
def get_optimizer(parameters, fp16, lr, momentum, weight_decay,
    ...
    if bn_weight_decay:
        ...
        optimizer = paddle.optimizer.Momentum(parameters, lr,
                                    momentum=momentum,
        ...
        optimizer = paddle.optimizer.Momentum([{'params': bn_params, 'weight_decay' : 0},
...
def get_train_step(model_and_loss, optimizer, fp16, use_amp = False, batch_size_multiplier = 1):
    def _step(input, target, optimizer_step = True):
        ...
        prec1, prec5 = paddle.zeros(1), paddle.zeros(1) #utils.accuracy(output.data, target, topk=(1, 5))
        ...
        paddle.static.cuda.synchronize()
...
train_max_ips = 0
train_max_batch = 0
def train(train_loader, model_and_loss, optimizer, lr_scheduler, fp16, logger, epoch, use_amp=False,
          prof=-1, batch_size_multiplier=1, register_metrics=True):
    ...
    for i, (input, target, index) in data_iter:     # NOTE: only needed for use_gradient
        ...
        loss, _, prec1, prec5 = model_and_loss(input, target, optimizer_step = optimizer_step)
        loss.backward()
        optimizer.step()
        optimizer.clear_grad()
...
def get_val_step(model_and_loss):
    def _step(input, target):
        ...
        prec1 = acc.accumulate()[0]
        if paddle.distributed.get_world_size() > 1:
            reduced_loss = paddle.distributed.all_reduce(loss)
            prec1 = paddle.distributed.all_reduce(prec1)
        else:
            reduced_loss = loss
        paddle.distributed.barrier()
        return reduced_loss, prec1, None
    return _step
...
def calc_ips(batch_size, time):
    world_size = paddle.distributed.get_world_size() if paddle.distributed.get_world_size() else 1
...
def train_loop(model_and_loss, optimizer, new_optimizer, lr_scheduler, train_loader, val_loader, debug_loader, epochs, fp16, logger,
    ...
    for epoch in epoch_iter:
        ...
        if save_checkpoints and (not paddle.distributed.get_world_size() or paddle.distributed.get_rank() == 0):
            if not skip_training:
                ...
                }, os.path.join(checkpoint_dir, 'model.pdparams'))
        # if not paddle.distributed.get_world_size() or paddle.distributed.get_rank() == 0:
        #     logger.end()
