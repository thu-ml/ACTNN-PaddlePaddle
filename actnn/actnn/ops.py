...
from paddle.nn.utils import _single, _pair, _triple


##################
from paddle.autograd import PyLayer as Function
##################
...
def quantize_and_pack(data, bits, mn, mx):
    if config.simulate:
        ...
        if isinstance(bits, int):  # Handle the case when config.adaptive_scheme is False
            bits = paddle.ones([N], dtype='int32', device='cuda') * bits
        B = (2 ** bits - 1).reshape([N, 1, 1])
        mn = mn - 1e-6
        mx = mx + 1e-6
        ...
        output = paddle.minimum(output, paddle.to_tensor(B, dtype='float32')).round().cast('int32')
    else:
        # Pack to bitstream
...
def quantize_activation(input, scheme):
    ...
    if input.dtype == paddle.float32:
        return q_input, q_bits, q_scale.astype('float16'), q_min.astype('float16')
    else:
...
def dequantize_activation(quantized, q_input_shape):
    ...
    if q_scale.dtype == paddle.float16:
        q_scale = q_scale.astype('float32')
        q_min = q_min.astype('float32')
...
class convnd(Function):
    ...
    def run_backward(n, ctx, grad_output, bias_reduce_dims, aug):
        ...
        if use_pipeline:
            ...
            input = paddle.chunk(input, micro_batch_size)
            grad_output = paddle.chunk(grad_output,  micro_batch_size)
            grad_weight = None
...
class linear(paddle.autograd.Function):
    @staticmethod
...
class batch_norm(paddle.autograd.Function):
    @staticmethod
    ...
    def backward(ctx, grad_output):
        ...
        quantized, weight, running_mean, running_var, save_mean, save_var, training, eps, reserve = ctx.saved_tensors
        q_input_shape = ctx.other_args
        input = dequantize_activation(quantized, q_input_shape)
        del quantized, ctx.saved_tensors
        empty_cache(config.empty_cache_threshold)
        if config.debug_memory_op_backward:
            global bn_layer_ct
...
class sync_batch_norm(paddle.autograd.Function):
    @staticmethod
    def forward(self, input, weight, bias, running_mean, running_var, eps, momentum, process_group, world_size, scheme):
        input = input.contiguous()
        count = paddle.empty([1], dtype=running_mean.dtype, device=input.device).fill_(input.numel() // input.shape[1])

        # calculate mean/invstd for input.
        mean, invstd = paddle.batch_norm_stats(input, eps)
        num_channels = input.shape[1]
        # C, C, 1 -> (2C + 1)
        combined = paddle.concat([mean, invstd, count], axis=0)
        # world_size * (2C + 1)
        combined_list = [
            paddle.empty_like(combined) for k in range(world_size)
        ]
        # Use allgather instead of allreduce since I don't trust in-place operations ..
        dist.all_gather(combined_list, combined, group=process_group, async_op=False)
        combined = paddle.stack(combined_list, axis=0)
        # world_size * (2C + 1) -> world_size * C, world_size * C, world_size * 1
        mean_all, invstd_all, count_all = paddle.split(combined, num_channels, axis=1)
        size = count_all.view([-1]).long().sum()
        if size == 1:
            raise ValueError('Expected more than 1 value per channel when training, got input size {}'.format(size))
        # calculate global mean & invstd
        mean, invstd = paddle.batch_norm_gather_stats_with_counts(
            input,
            mean_all,
            invstd_all,
            running_mean,
        ...
        return paddle.batch_norm_elemt(input, weight, bias, mean, invstd, eps)
    ...
    def backward(self, grad_output):
        ...
        sum_dy, sum_dy_xmu, grad_weight, grad_bias = paddle.fluid.layers.batch_norm_backward_reduce(
            grad_output,
            saved_input,
            mean,
            invstd,
        ...
        if self.needs_input_grad[0]:
            ...
            combined = paddle.concat([sum_dy, sum_dy_xmu], axis=0)
            paddle.distributed.all_reduce(
                combined, paddle.distributed.ReduceOp.SUM, process_group, use_calc_stream=True)
            sum_dy, sum_dy_xmu = paddle.split(combined, num_channels)
            divisor = count_tensor.sum()
            mean_dy = sum_dy / divisor
            mean_dy_xmu = sum_dy_xmu / divisor
            # backward pass for gradient calculation
            grad_input = paddle.fluid.layers.batch_norm_backward_elemt(
                grad_output,
                saved_input,
                mean,
                invstd,
...
class adaptive_avg_pool2d(paddle.autograd.Function):
    @staticmethod
    def forward(ctx, input, output_size):
        assert output_size == (1, 1)
        ctx.saved = input.shape
        return paddle.mean(input, axis=[2, 3], keepdim=True)
