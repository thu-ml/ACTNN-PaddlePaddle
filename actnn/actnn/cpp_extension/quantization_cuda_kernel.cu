...
std::pair<Tensor, Tensor> pack_mixed_precision_cuda(Tensor data,
  ...
  int max_bit = paddle::max(bits).item<int32_t>();
  int bits_per_int = sizeof(int32_t) * 8;
  PADDLE_ENFORCE_EQ(group_size % bits_per_int, 0, "group_size % bits_per_int == 0");
  // Compute total bits
  Tensor prefix_sum = paddle::cumsum(bits, 0, paddle::TensorOptions(paddle::kInt32).device(data.place()));
  int64_t total_bits = (int64_t)prefix_sum[-1].item<int32_t>() * num_groups * group_size;
  auto options = paddle::TensorOptions().dtype(paddle::kInt32).device(data.place());
  Tensor packed = paddle::empty({(total_bits + bits_per_int - 1) / bits_per_int,}, options);
  // Compute scale
  options = paddle::TensorOptions().dtype(data.dtype()).device(data.place());
  Tensor scale = paddle::empty({N, num_groups, 1}, options);
  int threads = 256;
  int blocks = (N * num_groups + threads - 1) / threads;
  PADDLE_DISPATCH_FLOATING_TYPES_AND_HALF(scale.scalar_type(), "compute_scale_mixed_precision", ([&] {
  ...
  auto gen = paddle::check_generator<paddle::CUDAGeneratorImpl>(paddle::platform::getDefaultCUDAGenerator(paddle::platform::get_cuda_device_id()));
  ...
  PADDLE_ENFORCE_EQ(stochastic, true, "stochastic must be true");
  // Call pack kernels
  ...
  for (int block_idx_y_base = 0; block_idx_y_base < logical_block_y_dim; block_idx_y_base += BLOCK_Y_DIM_MAX) {
    ...
    PADDLE_DISPATCH_FLOATING_TYPES_AND_HALF(data.scalar_type(), "pack_mixed_precision", ([&] {
...
Tensor unpack_mixed_precision_cuda(Tensor data,
                                   ...
                                   int64_t num_groups,
                                   int64_t group_size) {
  Tensor prefix_sum = paddle::cumsum(bits, 0, paddle::CUMSUM_EXCLUSIVE);

  auto options = paddle::TensorOptions().dtype(scale.type()).device(data.place());
  Tensor unpacked = paddle::empty({N, num_groups, group_size}, options);
  PADDLE_CHECK(group_size % 32 == 0);
  // Call unpack kernels
  int64_t logical_block_y_dim = N;
  for (int block_idx_y_base = 0; block_idx_y_base < logical_block_y_dim; block_idx_y_base += BLOCK_Y_DIM_MAX) {
    dim3 block_dim(num_groups, std::min(logical_block_y_dim - block_idx_y_base, BLOCK_Y_DIM_MAX), 1);
    dim3 thread_dim(group_size, 1, 1);
    paddle::platform::ForCUDAThreads(
        block_dim, thread_dim, [&] {
          unpack_mixed_precision_kernel<scalar_t><<<block_dim, thread_dim>>>(
              bits.data<int32_t>(), prefix_sum.data<int32_t>(),
              data.data<int32_t>(),
              scale.data<scalar_t>(), min.data<scalar_t>(),
              unpacked.data<scalar_t>(),
...
std::pair<Tensor, Tensor> pack_single_precision_cuda(Tensor data,
  ...
  PADDLE_ENFORCE_EQ(8 % bits, 0, "8 must be divisible by bits.");
  ...
  auto options = paddle::TensorOptions().dtype(paddle::kInt8).device(data.place());
  Tensor packed = paddle::empty({(total_bits + 8) / 8,}, options);
  // Compute scale
  options = paddle::TensorOptions().dtype(data.type()).device(data.place());
  Tensor scale = paddle::empty({N, num_groups, 1}, options);
  int threads = 256;
  int blocks = (N * num_groups + threads - 1) / threads;
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(data.type(), "compute_scale_single_precision", ([&] {
    compute_scale_single_precision_kernel<<<blocks, threads>>>(
      bits, min.data<scalar_t>(), max.data<scalar_t>(),
      scale.data<scalar_t>(), N, num_groups);
  }));
  // Random number generator
  auto gen = paddle::default_random_engine();
  std::pair<uint64_t, uint64_t> rng_engine_inputs;
  {
    // See Note [Acquire lock when using random generators]
  ...
  PADDLE_ENFORCE(stochastic, "stochastic must be true.");
  // Call pack kernels
  ...
  for (int64_t block_idx_y_base = 0; block_idx_y_base < logical_block_y_dim; block_idx_y_base += BLOCK_Y_DIM_MAX) {
    ...
    if (N % work_per_thread == 0) {
      AT_DISPATCH_FLOATING_TYPES_AND_HALF(data.type(), "pack_single_precision", ([&] {
...
Tensor unpack_single_precision_cuda(Tensor data,
  ...
  Tensor unpacked = paddle::empty({N, num_groups, group_size}, options);
  int work_per_thread = 8 / bits;
  PADDLE_ENFORCE_EQ(8 % bits, 0, paddle::platform::errors::InvalidArgument("8 % bits should be 0."));
  // Call unpack kernels
  int64_t logical_block_y_dim = (N + work_per_thread - 1) / work_per_thread;
  for (int64_t block_idx_y_base = 0; block_idx_y_base < logical_block_y_dim; block_idx_y_base += BLOCK_Y_DIM_MAX) {
    dim3 block_dim(num_groups, std::min(logical_block_y_dim - block_idx_y_base, BLOCK_Y_DIM_MAX), 1);
    dim3 thread_dim(group_size, 1, 1);
    if (N % work_per_thread == 0) {
      PADDLE_DISPATCH_FLOATING_TYPES_AND_HALF(scale.type(), "unpack_single_precision", ([&] {
...
std::pair<Tensor, Tensor> act_quantized_relu_forward_cuda(Tensor data) {
  ...
  Tensor mask = paddle::empty({mask_len}, options);
  Tensor output = paddle::empty_like(data);
  ...
  act_quantized_relu_forward_kernel<scalar_t><<<blocks, threads>>>(
    data.data<scalar_t>(), mask.data<int32_t>(), output.data<scalar_t>(),
    n_elements, mask_len);
...
Tensor act_quantized_relu_backward_cuda(Tensor grad_output, Tensor mask) {
  ...
  int threads = ACT_QUANTIZED_RELU_NUM_THREADS;
  int blocks = (n_elements + threads - 1) / threads;
  Tensor grad_input = paddle::empty_like(grad_output);
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(grad_output.type(), "act_quantized_relu_backward", ([&] {
      act_quantized_relu_backward_kernel<scalar_t><<<blocks, threads>>>(
        grad_output.data<scalar_t>(), mask.data<int32_t>(), grad_input.data<scalar_t>(),
        n_elements);
  }));
  return grad_input;
}
...
std::pair<Tensor, Tensor> act_quantized_dropout_forward_cuda(Tensor data, float dropout_p) {
  ...
  int blocks = (n_elements + threads - 1) / threads;
  // Random number generator
  auto gen = paddle::check_generator<paddle::CUDAGeneratorImpl>(paddle::platform::GetDefaultCUDAGenerator(data.place()));
  std::pair<uint64_t, uint64_t> rng_engine_inputs;
  {
    // See Note [Acquire lock when using random generators]
    std::lock_guard<std::mutex> lock(gen->mutex_);
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(data.type(), "act_quantized_dropout_forward", ([&] {
...
__global__ void act_quantized_max_pool2d_forward_kernel(const scalar_t* __restrict__ input,
  ...
  if (id < n_elements) {
    ...
    int w = id % W_out;
    int h_base = h * SH - PH;
    int h_start = fmax(h_base, 0);
    int h_end = fmin(h_base + KH, H);
    int w_base = w * SW - PW;
    int w_start = fmax(w_base, 0);
    int w_end = fmin(w_base + KW, W);
    scalar_t v = -1e10;
    int8_t index;
...
std::pair<Tensor, Tensor> act_quantized_max_pool2d_forward_cuda(Tensor input,
  ...
  Tensor output = paddle::empty({N, C, H_out, W_out}, dtype);
  dtype = paddle::DataType::INT8;
  PADDLE_ENFORCE(!ceil_mode, "Do not support ceil_mode");
  PADDLE_ENFORCE(!return_indices, "Do not support return_indices");
  Tensor max_indices = paddle::empty({N, C, H_out, W_out}, dtype);
  ...
  act_quantized_max_pool2d_forward_kernel<scalar_t><<<blocks, threads>>>(
      input.data<scalar_t>(), output.data<scalar_t>(), max_indices.data<int8_t>(), n_elements,
...
Tensor act_quantized_max_pool2d_backward_cuda(Tensor grad_output, Tensor max_indices,
        ...
        IntArrayRef kernel_size, IntArrayRef stride, IntArrayRef padding, IntArrayRef dilation,
        bool ceil_mode, bool return_indices) {
  auto options = paddle::TensorOptions().dtype(grad_output.dtype()).device(grad_output.place());
  Tensor grad_input =  paddle::zeros(input_shape, options);
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(grad_output.scalar_type(), "act_quantized_max_pool2d_backward", ([&] {
    act_quantized_max_pool2d_backward_kernel<scalar_t><<<blocks, threads>>>(
      grad_output.data<scalar_t>(), max_indices.data<int8_t>(), grad_input.data<scalar_t>(),
      n_elements,
      N, C, H, W, H_out, W_out, kernel_size[0], kernel_size[1], stride[0], stride[1],
      padding[0], padding[1], dilation[0], dilation[1]);
  }));
