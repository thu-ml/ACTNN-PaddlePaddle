# The code is compatible with PaddlePaddle 2.0
...
from paddle.nn.modules.pooling import _size_2_t, _single, _pair, _triple, MaxPool1D, MaxPool2D, MaxPool3D, AvgPool1D, AvgPool2D, AvgPool3D

from paddle.nn.modules.pooling import _MaxPoolNd, _AvgPoolNd

from paddle.nn.functional import max_pool2d, avg_pool2d
...
class QConv2d(nn.Conv2D):
    def __init__(self, in_channels, out_channels, kernel_size,
        ...
        super(QConv2d, self).__init__(in_channels, out_channels, kernel_size,
                                      stride, padding, dilation, groups, bias_attr=bias, padding_mode=padding_mode)
        if isinstance(kernel_size, int):
...
class QConvTranspose2d(nn.Conv2DTranspose):
    def __init__(self, in_channels, out_channels, kernel_size,
                 stride=1, padding=0, output_padding=0, groups=1,
                 bias=True, dilation=1, padding_mode='zeros', group=0):
...
class QConvTranspose3d(nn.Conv3DTranspose):
    def __init__(self, in_channels, out_channels, kernel_size,
                 stride=1, padding=0, output_padding=0, groups=1,
                 bias=True, dilation=1, padding_mode='zeros', group=0):
...
class QBatchNorm3d(nn.BatchNorm3d):
    ...
    def forward(self, input):
        ...
        return batch_norm.apply(
            input,
            # If buffers are not to be tracked, ensure that they won't be updated
            self._mean if not self.training or self.track_running_stats else None,
            self._variance if not self.training or self.track_running_stats else None,
            self.weight, self.bias, bn_training, exponential_average_factor, self.eps, self.scheme)


class QReLU(nn.Layer):
...
class QSyncBatchNorm(nn.SyncBatchNorm):
    ...
    def forward(self, input):
        ...
        r"""
        Buffers are only updated if they are to be
        tracked and we are in training mode. Thus they only need to be
        passed when the update should occur (i.e. in training mode when they are tracked), or when buffer stats are
        used for normalization (i.e. in eval mode when buffers are not None).
        """
        # If buffers are not to be tracked, ensure that they won't be updated
        assert self.running_mean is None or isinstance(self.running_mean, paddle.Tensor)
        assert self.running_var is None or isinstance(self.running_var, paddle.Tensor)
        ...
        if need_sync:
            process_group = dist.ParallelEnv().world_size()
            if self.process_group:
                process_group = self.process_group
            world_size = dist.ParallelEnv().world_size()
        ...
        if not need_sync:
            ...
            if not self.ddp_gpu_size:
                raise AttributeError('SyncBatchNorm is only supported within paddle.nn.parallel.DistributedDataParallel')
            assert bn_training
...
class QAvgPool2d(nn.AvgPool2D):
    ...
    def __init__(self, kernel_size: _size_2_t, stride: Optional[_size_2_t] = None, padding: _size_2_t = 0,
        ...
        super().__init__(kernel_size=kernel_size, stride=stride, padding=padding, ceil_mode=ceil_mode, count_include_pad=count_include_pad, divisor_override=divisor_override)
